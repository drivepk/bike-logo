...
class ImageOptimizer extends BaseClass {
  ...
  constructor() {
    super();
    this.ensureTargetDirectories();
    this.loadOriginals();
  }

  protected ensureTargetDirectories() {
    [LogosTargetLocation.Original, LogosTargetLocation.Optimized, LogosTargetLocation.Thumbs].forEach(
      (dir) => {
  ...
  protected loadOriginals() {
    if (!fs.existsSync(LogosTargetLocation.Original)) {
      throw new Error(`Original logos directory not found: ${LogosTargetLocation.Original}`);
    }
  ...
  protected async optimizeImages() {
    for (const logo of this.originals) {
      ...
      const slug = this.slugify(name, { lower: true });
      const targetFile = `${slug}.png`;
